...
function joinUrlQuery(queryname, queryparams, firstquery, appendifempty) {
    ...
    let _join = (name, params) => ((0, convertToArray_1.convertToArray)(params, false).length > 0 || appendifempty) ? `${name}=${(0, convertToArray_1.convertToArray)(params, false).join(`&${name}=`)}` : undefined;
    if (queryparams_.length == 0)
        return "";
    let r = `${(firstquery ? "?" : "&")}`;
    r += ((queryname_.length === 1) ? _join(queryname_[0], queryparams_) : `${queryname_.map((v, i) => _join(v, queryparams_[i])).filter(a => a).join("&")}`);
    return r;
}
