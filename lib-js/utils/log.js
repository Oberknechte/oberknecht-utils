...
function log(logopt, logmsg, logcolorfg, logcolorbg) {
    ...
    if (typeof logmsg === "string") {
        if (global.logs)
            global.logs.all[Date.now()] = global.logs[String(logopt)][Date.now()] = [logopt, logmsg?.replace(/\x1b\[[\w;]+m/g, ""), date_()];
    }
    ;
    if (!logmsg && logopt) {
        logmsg = logopt;
    ...
    switch (logopt) {
        default:
        case 0: return console.log(...logm);
        case 1: return console.info(...logm);
        case 2: return console.error(...logm);
    }
    ;
}
exports.log = log;
