...
export function joinUrlQuery(queryname: string | Array<string>, queryparams: string | Array<string>, firstquery?: boolean, appendifempty?: boolean) {
    ...
    let _join = (name: string, params: Array<string>): string | undefined => (convertToArray(params, false).length > 0 || appendifempty) ? `${name}=${convertToArray(params, false).join(`&${name}=`)}` : undefined;
