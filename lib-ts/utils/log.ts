...
export function log(logopt: Number | String, logmsg?: any, logcolorfg?: String, logcolorbg?: String): void {
    ...
    if (typeof logmsg === "string") {
        if (global.logs) global.logs.all[Date.now()] = global.logs[String(logopt)][Date.now()] = [logopt, logmsg?.replace(/\x1b\[[\w;]+m/g, ""), date_()];
    };
    if (!logmsg && logopt) {
        logmsg = logopt;
        logopt = 0;
